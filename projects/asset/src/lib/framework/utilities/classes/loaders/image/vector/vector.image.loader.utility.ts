...
import { IVectorImage } from '@imaging/domain/primitives/interfaces/images/vector/vector.image.asset.primitive.interface';

import { AssetLoaded } from '@imaging/domain/states/asset/state/asset.state';


export class VectorImageLoader implements IAssetLoader<IVectorImage>
{
  ...
  public load$(asset: IVectorImage): Observable<IVectorImage>
  {
    ...
    const parser: IAssetParser<IVectorImage> = new SvgImageParser();

    return this.httpClient.get(uri, { headers, responseType: 'text' })
      .pipe(mergeMap((data: string): Observable<IVectorImage> =>
      {
        asset.data = data;
        return parser.parse$(asset, new DOMParser())
          .pipe(mergeMap((asset: IVectorImage): Observable<IVectorImage> =>
          {
            if (!asset.errored)
            {
      ...
      .pipe(catchError((error: any): Observable<IVectorImage> =>
      {
        return of(asset.fail('An error occurred reading the images', error) as IVectorImage);
      }));
  }
